...
var DatePicker = new Class({
	...
	formatMinMaxDates: function() {
	    if (this.options.minDate) {
                if (this.options.minDate.format && this.options.minDate.date) {
		    this.options.minDate = this.unformat(this.options.minDate.date, this.options.minDate.format);
		}
                else if (this.options.minDate == 'now' || this.options.minDate == 'today') {
                    this.options.minDate = new Date();
                    this.options.minDate.set({'hr': 0, 'min': 0, 'sec': 0});
                }
            }
	    if (this.options.maxDate) {
                if (this.options.maxDate.format && this.options.maxDate.date) {
		    this.options.maxDate = this.unformat(this.options.maxDate.date, this.options.maxDate.format);
		    this.options.maxDate.setHours(23);
		    this.options.maxDate.setMinutes(59);
		    this.options.maxDate.setSeconds(59);
                }
                else if (this.options.maxDate == 'now' || this.options.maxDate == 'today') {
                    this.options.maxDate = new Date();
                    this.options.minDate.set({'hr': 23, 'min': 59, 'sec': 59});
                }
            }
	},
	...
	fx: function(fx) {
		if (fx == 'right') {
			...
			this.oldContents.setStyle('left', 0).set('tween',
			                                         { onComplete: function() { this.oldContents.setStyle('left', this.bodysize.x); }.bind(this),
			                                           duration: this.options.animationDuration / 2 }).tween('opacity', 1, 0);
